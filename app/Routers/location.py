...
@router.post("/", response_model=schemas.LocationResponse, tags=["Location"])
...
@router.get('/{location_id}', response_model=schemas.LocationResponse, tags=["Location"])
...
@router.get('/', response_model=list[schemas.LocationResponse], tags=["Location"])
async def read_locations(skip: int = 0, limit: int = 10, current_user: schemas.User = Depends(auth.get_current_user),db: AsyncSession = Depends(get_db)):
    db_locations = await crud.get_locations(skip=skip, limit=limit, db=db)
    return db_locations

@router.delete('/{location_id}', status_code=status.HTTP_204_NO_CONTENT, tags=["Location"])
